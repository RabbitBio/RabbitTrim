...
int
pragzipCLI( int argc, char** argv, PragzipQueue& Q)
{
    /**
    ...
    if ( decompress || countBytes || countLines || !args.indexSavePath.empty() ) {
        ...
        if ( args.decoderParallelism == 1 ) {
            ...
            const auto writeAndCount =
                [outputFileDescriptor, countLines, &newlineCount, &Q]
                ( const std::shared_ptr<pragzip::ChunkData>& chunkData,
                  size_t const                               offsetInBlock,
                  size_t const                               dataToWriteSize )
                {
                    writeAll( chunkData, outputFileDescriptor, offsetInBlock, dataToWriteSize, Q);
                    if ( countLines ) {
...
int
main_pragzip( int argc, char** argv, PragzipQueue& Q)
{
    for(int i = 0; i < argc; i++) printf("%s ", argv[i]);
    printf("\n");
    try
    {
        return pragzipCLI( argc, argv, Q);
    }
