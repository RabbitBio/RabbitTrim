...
// new writeAll func used in rtrim
inline void
writeAll( const std::shared_ptr<ChunkData>& chunkData,
         ...
         PragzipQueue& Q
         // moodycamel::ReaderWriterQueue<std::pair<char *, int>> *Q, std::atomic_int *producerDone
         )
{
          ...
          static_cast<bool>( it ); ++it )
    {
        ...
        if ( !splicable ) {
            writeAllToFd( outputFileDescriptor, buffer, size, Q);
        }
    }
#endif
}
