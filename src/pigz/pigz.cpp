...
local void parallel_compress(rabbit::trim::WriterBufferDataPool* wbDataPool, rabbit::trim::WriterBufferDataQueue& dq2, std::pair<char*,int>& L) {
  ...
  next->len = readFromQueue(wbDataPool, dq2, L, next->buf, next->size);
  hold = NULL;
  dict = NULL;
  scan = next->buf;
  hash = RSYNCHIT;
...
local void process(char* path, rabbit::trim::WriterBufferDataPool* wbDataPool, rabbit::trim::WriterBufferDataQueue& dq2, std::pair<char*, int>& L){
  ...
  if (path == NULL) {
    ...
    if ((st.st_mode & S_IFMT) == S_IFDIR) {
      ...
      for (off = 0; roll[off]; off += strlen(roll + off) + 1) {
        ...
        process(g[small_map[*((int *) (pthread_getspecific(gtid)))]].inf, wbDataPool, dq2, L);
      }
      g[small_map[*((int *) (pthread_getspecific(gtid)))]].inf[len] = 0;
  ...
  if (g[small_map[*((int *) (pthread_getspecific(gtid)))]].decode) {
    ...
    parallel_compress(wbDataPool, dq2, L);
#endif
  else
    single_compress(0);
  if (g[small_map[*((int *) (pthread_getspecific(gtid)))]].verbosity > 1) {
  ...
  try
  {
    ...
    for (n = 1; n < argc; n++)
      if (argv[n] != NULL) {
        ...
        process(n < nop && strcmp(argv[n], "-") == 0 ? NULL : argv[n], wbDataPool, dq2, L);
        done++;
      }
    // list stdin or compress stdin to stdout if no file names provided
    if (done == 0)
      process(NULL, wbDataPool, dq2, L);
  }
  always
  {
