...
bool rabbit::trim::util::endsWith(string const &value, string const &ending) {
  if (ending.size() > value.size()) return false;
  return equal(ending.rbegin(), ending.rend(), value.rbegin());
}

vector<string> rabbit::trim::util::split(const string& str, const string& delim) {
	vector<string> res;
	if("" == str) return res;
...
string& rabbit::trim::util::ClearHeadTailSpace(string &str)   
{  
    if (str.empty())   
    {  
        return str;  
...
bool rabbit::trim::util::isFileExist(const char* filename){
	struct stat buffer;   
  	return (stat(filename, &buffer) == 0); 
}

// format
int rabbit::trim::util::neoGetLine(rabbit::fq::FastqDataChunk *&chunk, uint64_t &pos, uint64_t &len) {
  int start_pos = pos;
...
//int rabbit::trim::util::chunkFormat(rabbit::fq::FastqDataChunk *fqDataChunk, CSEREAD *read, bool mHasQuality = true) {
...
//unsigned int rabbit::trim::util::chunkFormat_PE(rabbit::fq::FastqDataPairChunk* chunk,CPEREAD * read, bool mHasQuality = true){
...
// 多线程时间统计
double rabbit::trim::util::getTime(){
	struct timeval tv;
...
// extract file names 
void rabbit::trim::util::extractFileNames(const char *str, vector<string>& Rs) {
	string fileName = "";
...
//unsigned int rabbit::trim::util::load_batch_data_SE_C( FILE * fp, CSEREAD *loadingReads, unsigned int num ) {
...
//}
//
//unsigned int rabbit::trim::util::load_batch_data_SE_GZ( gzFile gfp, CSEREAD *loadingReads, unsigned int num ) {
//	register unsigned int loaded = 0;
////	string unk;
//	register CSEREAD *p = loadingReads;
//	register CSEREAD *q = p + num;
...
//}
//
//unsigned int rabbit::trim::util::load_batch_data_PE_C( FILE *fq1, FILE *fq2, CPEREAD *loadingReads, unsigned int num ) {
//	//register unsigned int loaded = 0;
...
//}
//
//unsigned int rabbit::trim::util::load_batch_data_PE_GZ( gzFile gfp1, gzFile gfp2, CPEREAD *loadingReads, unsigned int num ) {
//	//register unsigned int loaded = 0;
//	register CPEREAD *p = loadingReads;
//	register CPEREAD *q = p + num;
//	register CPEREAD *s = p;
...
//*/
//bool rabbit::trim::util::check_mismatch_dynamic_SE_C( CSEREAD *read, unsigned int pos, const ktrim_param & kp ) {
...
//*/
//bool rabbit::trim::util::check_mismatch_dynamic_PE_C( const CPEREAD *read, unsigned int pos, const ktrim_param &kp ) {
...
//int rabbit::trim::util::get_quality_trim_cycle_se( const char *p, const int total_size, const ktrim_param &kp ) {
//	register int i, j, k;
...
//}
//
//int rabbit::trim::util::get_quality_trim_cycle_pe( const CPEREAD *read, const ktrim_param &kp ) {
//	register int i, j, k;
//	register int stop = kp.min_length - 1;
