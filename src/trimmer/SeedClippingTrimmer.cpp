...
void SeedClippingTrimmer::processPairRecord(neoReference& rec1, neoReference& rec2)
{
    ...
    {
        indexloc = std::strstr(indexloc, adapter_index_1);
        if(indexloc != NULL) seed.emplace(indexloc - rec1_seq);
        else break;
    }
    indexloc = rec2_seq;
    while(true)
    {
        indexloc = std::strstr(indexloc, adapter_index_2);
        if(indexloc != NULL) seed.emplace(indexloc - rec2_seq);
        else break;
    }
    ...
    if(!is_revcomp(rec1_seq[5], rec2_seq[rec_len - 6])) tmpMiss++;
    if(!is_revcomp(rec2_seq[5], rec1_seq[rec_len - 6])) tmpMiss++;
    if(tmpMiss <= 1)
    {
        rec1.lorigin = 2;
        rec2.lorigin = 2;
    ...
    else
    {
        ...
        if(!is_revcomp(rec1_seq[5], rec2_seq[rec_len - 6])) tmpMiss++;
        if(!is_revcomp(rec2_seq[5], rec1_seq[rec_len - 6])) tmpMiss++;
        if(!is_revcomp(rec1_seq[6], rec2_seq[rec_len - 7])) tmpMiss++;
        if(!is_revcomp(rec2_seq[6], rec1_seq[rec_len - 7])) tmpMiss++;
        if(tmpMiss <= 1){
            rec1.lorigin = 2;
            rec2.lorigin = 2;
            rec_len -= 1;
